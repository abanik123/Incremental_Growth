...
code_m <- nimbleCode({
  ...
  mu_o <- 1.1     # Since variaility in growth rate is high, alpha < 5
  sigma_mu_o <- 3 # And, for Gamma variance 10
  
  a ~ T(dnorm(6.67, 0.1),6.54, 6.8)
  ...
  for (i in 1:N) {
    ...
    for (t in (first[i]+1):nyears) {
      ...
      fl[i, t] ~ T(dnorm(fl[i, t-1] + k[t], sd = sigma), fl[i,t-1], a)
...
#------------------------------------------------------------------------#
parameters.to.save <- c("alpha", "beta", "sigma", "k", "a", "fl")
...
write.csv(combined_samples, file = "Big_m1_samples_S.csv", row.names = FALSE)

pdf(file = "Big_mp_m1_S.pdf")
MCMCplot(samples, HPD = T)
dev.off()

s <- MCMCsummary(samples, round = 5, params = c("alpha", "beta", "sigma", "k", "a"))

MCMCtrace(samples,pdf = T,open_pdf = F,filename = "Big_m1_S", ind = TRUE,
          Rhat = FALSE, n.eff = FALSE)
write.csv(s, file = "Big_m1_sum_S.csv")
...
write.csv(f, file = "Big_m1_length_s.csv")
