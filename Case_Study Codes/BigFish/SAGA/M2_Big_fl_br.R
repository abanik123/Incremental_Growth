...
code_m <- nimbleCode({
  ...
  for (i in 1:N) {
    ...
    for (t in (first[i]+1):nyears) {
      ...
      fl[i, t] ~ T(dnorm(fl[i, t-1] + k[i,t], sigma), fl[i,t-1], a)
...
#------------------------------------------------------------------------#
parameters.to.save <- c("alpha", "beta", "psi", "a", "sigma","k_sp","k_sl", "fl")
...
write.csv(combined_samples, file = "Big_m2_samples_s.csv", row.names = FALSE)

pdf(file = "Big_mp_m2_s.pdf")
MCMCplot(samples, HPD = T)
dev.off()

s <- MCMCsummary(samples, round = 5, params = c("alpha", "beta", "psi", "a", "sigma","k_sp","k_sl"))

MCMCtrace(samples,pdf = T,open_pdf = F,filename = "Big_m2_s", ind = TRUE,
          Rhat = FALSE, n.eff = FALSE)
write.csv(s, file = "Big_m2_sum_s.csv")
...
write.csv(f, file = "Big_m2_length_s.csv")
