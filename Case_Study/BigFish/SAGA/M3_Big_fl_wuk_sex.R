...
write.csv(combined_samples, file = "Big_m3_samples_S.csv", row.names = FALSE)

pdf(file = "Big_mp_m3_s.pdf")
MCMCplot(samples, HPD = T)
dev.off()

s <- MCMCsummary(samples, round = 5, params = c("alpha", "beta", "sigma", "Linf_f",
                                                "Linf_m","k_m","k_f","p.male"))

MCMCtrace(samples,pdf = T,open_pdf = F,filename = "Big_m3_s", ind = TRUE,
          Rhat = FALSE, n.eff = FALSE)
write.csv(s, file = "Big_m3_sum_s.csv")
...
write.csv(f, file = "Big_m3_length_s.csv")
