...
code_m <- nimbleCode({
  ...
  # asymptotic size
  a ~  T(dnorm(6.67, 0.1),6.54, 6.8) # Anadromous DV's length as always lies between 300 (ln(300) = 5.7) to 900 (ln(900) = 6.8).
  #a ~ dunif(6.8, 7)
  # growth coefficient
...
write.csv(combined_samples, file = "Big_m1_samples_VB.csv", row.names = FALSE)

pdf(file = "Big_mp_m1_VB.pdf")
MCMCplot(samples, HPD = T)
dev.off()

s <- MCMCsummary(samples, round = 5, params = c("alpha", "beta", "a","k","sigma"))

MCMCtrace(samples,pdf = T,open_pdf = F,filename = "Big_m1_VB", ind = TRUE,
          Rhat = FALSE, n.eff = FALSE)

write.csv(s, file = "Big_m1_sum_VB.csv")
...
write.csv(f, file = "Big_m1_length_VB.csv")
