...
code_m <- nimbleCode({
  ...
  #sigma_mu_o <- 100
  sigma_mu_o <- 3 # For Gamma variance 10
  
  a ~  T(dnorm(6.68, 0.1), 6.72, 6.75)
...
write.csv(combined_samples_nofl, file = "Joe_m1_samples_S.csv", row.names = FALSE)
...
pdf(file = "Joe_mp_m1_S.pdf")
MCMCplot(samples_nofl, HPD = TRUE)
dev.off()

# MCMC summary (no fl)
s <- MCMCsummary(samples_nofl, round = 5, params = c("alpha", "beta", "sigma", "k", "a"))
write.csv(s, file = "Joe_m1_sum_S.csv")

# Trace plot (no fl)
MCMCtrace(samples_nofl, pdf = TRUE, open_pdf = FALSE, filename = "Joe_m1_S", ind = TRUE,
...
write.csv(f, file = "Joe_m1_length_s.csv")
