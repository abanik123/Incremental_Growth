...
#------------------------------------------------------------------------#
parameters.to.save <- c("alpha", "beta", "psi", "a", 
                        "sigma","k_sp","k_sl", "fl")
...
write.csv(combined_samples, file = "Joe_m2_samples.csv", row.names = FALSE)

pdf(file = "Joe_mp_m2.pdf")
MCMCplot(samples, HPD = T)
dev.off()

s <- MCMCsummary(samples, round = 5, params = c("alpha", "beta", "psi", "a", 
                                                "sigma","k_sp","k_sl"))

MCMCtrace(samples,pdf = T,open_pdf = F,filename = "Joe_m2", ind = TRUE,
          Rhat = FALSE, n.eff = FALSE)
write.csv(s, file = "Joe_m2_sum.csv")
...
write.csv(f, file = "Joe_m2_length_s.csv")
