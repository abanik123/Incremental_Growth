...
code_m <- nimbleCode({
  ...
  # asymptotic size
  a ~  T(dnorm(6.76, 0.1), 6.73, 6.8) # Anadromous DV's length as always lies between 300 (ln(300) = 5.7) to 900 (ln(900) = 6.8).
  #a ~ dunif(6.8, 7)
  # growth coefficient
  
  for (t in 1:nyears){
    
    k[t] ~ dgamma(shape = 1.1, scale = 3) # Variance 10
  }
  
  for (i in 1:N) {
    
    # first capture
    #P[i,first[i]] <- lambda*H_m[i,first[i]] # this multiplies individual heterogeneity (lambda) by expected fl
    #H_m[i,first[i]] ~ dunif(5.5,6.75)   # expected size of animal at unknown age at first capture
    
    for (t in (first[i]+1):nyears) {
      
      fl[i,t] ~ T(dnorm(increment[i,t],sigma), fl[i,t-1],a)
      increment[i,t] <- fl[i,t-1] + (a - fl[i,t-1])*(1-exp(-k[t]))  # VB Model
    }
...
initial.values <- list(alpha = runif(2, 0, 1), beta = runif(2,0,1),
                       a = 6.76, k = k_inits, 
                       increment = increment_inits,
                       sigma = runif(1, 0, 10), a_sigma = 2.1, b_sigma = 1.1,
                       fl = fl_inits, z = z_inits)


#' Parameters to monitor. 
#------------------------------------------------------------------------#
parameters.to.save <- c("alpha", "beta", "a","k","sigma","fl")
...
write.csv(combined_samples, file = "Joe_m1_samples_VB.csv", row.names = FALSE)

pdf(file = "Joe_mp_m1_VB.pdf")
MCMCplot(samples, HPD = T)
dev.off()

s <- MCMCsummary(samples, round = 5, params = c("alpha", "beta", "a","k","sigma"))

MCMCtrace(samples,pdf = T,open_pdf = F,filename = "Joe_m1_VB", ind = TRUE,
          Rhat = FALSE, n.eff = FALSE)

write.csv(s, file = "Joe_m1_sum_VB.csv")
...
write.csv(f, file = "Joe_m1_length_VB.csv")
